...
function add_to_xcode_project() {
    local target_name="$1"           # 앱 타겟
    local test_target_name="$2"       # 테스트 타겟
    local solution_file_name="$3.swift"
    local ps_platform="$4"
    local difficulty="$5"
    
    if [ -n "$difficulty" ]; then
    ./scripts/add_to_xcode_project.rb "$XCODE_PROJECT_NAME" "$target_name" "$test_target_name" "$ps_platform/$difficulty/$solution_file_name" "$ps_platform" "$difficulty"
    else
    ./scripts/add_to_xcode_project.rb "$XCODE_PROJECT_NAME" "$target_name" "$test_target_name" "$ps_platform/$solution_file_name" "$ps_platform"
    fi
    
    echo "✅ Added Swift file to Xcode project: $solution_file_name"
}
